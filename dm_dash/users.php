...
// add
if(isset($_POST['add'])) {
    if(isset($_POST['firstname'], $_POST['lastname'], $_POST['phone'], $_POST['email'], $_POST['password'], $_POST['username'])) {
        if(!empty($_POST['firstname']) && !empty($_POST['lastname']) && !empty($_POST['username']) && !empty($_POST['password'])) {
            if(!empty($_POST['phone']) && validatePhoneNumber($_POST['phone']) == false) 
            {
                ...
                if(!empty($_POST['email']) && isValidEmail($_POST['email']) == false) {
                    ...
                    try {
                        ...
                        $stmt = $cn->prepare("INSERT INTO users (firstname, lastname, phone, email, status, username, password) 
                                              VALUES (:firstname, :lastname, :phone, :email, :status, :username, :password)");
                        $status = 1;
                                ...
                                <div class="live-preview">
                                    <div class="table-responsive">
                                        <table class="table align-middle table-nowrap mb-0">
                                            ...
                                            <tbody>
                                                <?php $i = 1; while($row = $users->fetch(PDO::FETCH_ASSOC)) { ?>
                                                <tr>
